...
try:
    ...
    print('Need package pytorch_wavelets for MLWBDN!!! try to run pip install pytorch-wavelet')
...
class Denselayer(nn.Module):
    ...
    def __init__(self, in_chs, num_layer, growth_rate, bottle_neck=0):
        ...
        if self.bottle_neck == 0:
            self.conv3x3 = nn.Conv2d(self.in_chs, self.growth_rate, kernel_size=3, padding=1, stride=1,bias=False)
            self.bn1 = nn.BatchNorm2d(self.growth_rate)
            self.relu1 = nn.LeakyReLU()
        else: 
            self.bottle_size = self.bottle_neck*self.growth_rate
            self.conv1x1 = nn.Conv2d(self.in_chs, self.bottle_size, kernel_size=1, padding=0, stride=1,bias=False)
            self.bn1 = nn.BatchNorm2d(self.bottle_size)
            self.relu1 = nn.LeakyReLU()
            self.conv3x3 = nn.Conv2d(self.bottle_size, self.growth_rate, kernel_size=3, padding=1, stride=1,bias=False)
...
class SADFBlock(nn.Module):
    ...
    def __init__(self, in_chs, growth_rate, bottle_neck_list=[0,0,4,4], layercount=4):
        ...
        for i in range(self.layercount):
            ...
            # self.layer.add_module('Denslayer{}'.format(i+1),SepDenselayer(self.in_chs[i], i+1, self.growth_rate, self.bottle_neck[i]))
            self.layer.add_module('Denslayer{}'.format(i+1),GhoDenselayer(self.in_chs[i], i+1, self.growth_rate, self.bottle_neck[i]))
...
class GhoDenselayer(nn.Module):
    ...
    def __init__(self, in_chs, num_layer, growth_rate, bottle_neck=0):
        ...
        if self.bottle_neck == 0:
            self.GhostModule = GhostModule(self.in_chs, self.growth_rate, kernel_size=1, ratio=2, dw_size=3,stride=1,relu=True)
        else: 
            self.bottle_size = self.bottle_neck*self.growth_rate
            self.GhostModule1 = GhostModule(self.in_chs, self.bottle_size, kernel_size=1, ratio=2, dw_size=3,stride=1,relu=True)
            self.GhostModule2 = GhostModule(self.bottle_size, self.growth_rate, kernel_size=1, ratio=2, dw_size=3,stride=1,relu=True)
    ...
    def forward(self, x):
        ...
        if self.bottle_neck == 0:
            ...
            x = self.GhostModule1(x)
            x = self.GhostModule2(x)
        return x
...
class DSConvBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size = 3,stride=1,padding = 1,Bias = True):
        ...
        self.conv1 = nn.Conv2d(in_channels, in_channels, kernel_size=kernel_size, stride=stride, padding=padding, groups=in_channels, bias=Bias)
        self.bn1 = nn.BatchNorm2d(in_channels)
        self.conv2 = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, padding=0, bias=Bias)
...
class SepDenselayer(nn.Module):
    ...
    def __init__(self, in_chs, num_layer, growth_rate, bottle_neck=0):
        ...
        if self.bottle_neck == 0:
            self.conv3x3 = DSConvBlock(self.in_chs, self.growth_rate, kernel_size=3, padding=1, stride=1,Bias=False)
            self.bn1 = nn.BatchNorm2d(self.growth_rate)
            self.relu1 = nn.LeakyReLU()
        else: 
            self.bottle_size = self.bottle_neck*self.growth_rate
            ...
            self.bn1 = nn.BatchNorm2d(self.bottle_size)
            self.relu1 = nn.LeakyReLU()
            self.conv3x3 = DSConvBlock(self.bottle_size, self.growth_rate, kernel_size=3, padding=1, stride=1,Bias=False)
            self.bn2 = nn.BatchNorm2d(self.growth_rate)
            self.relu2 = nn.LeakyReLU()
...
class classifier(nn.Module):
    def __init__(self, in_chs, out_num, bn = False, block_num = 1):
        ...
        self.conv1x1_1 = nn.Conv2d(self.in_chs, tmpchs, 1, stride=1, padding=0,bias=False)
        self.conv3x3_1 = nn.Conv2d(tmpchs, tmpchs, 3, stride=1, padding=1,groups=tmpchs,bias=False)
        self.conv3x3_2 = nn.Conv2d(tmpchs, out_num, 3, stride=1, padding=1,bias=False)
        self.relu1 = nn.LeakyReLU()
        self.relu2 = nn.LeakyReLU()
...
class MFFBlock(nn.Module):
    def __init__(self, in_chs, cmp_rate, wavelet='haar', WTpadding_mode = 'periodization',bn=False, growth_rate=64, bottle_neck=[0,0,4,4], block_num = 1):
        ...
        self.Transitionlayer_LL = nn.Sequential(
            nn.Conv2d(in_chs, out_chs, kernel_size=1, padding=0, stride=1,bias=False),
        ...
        # self.SADFBlock = SADFBlockv2(out_chs, growth_rate, bottle_neck)
...
class MLWBDN(nn.Module):
    def __init__(self, in_chs, num_class, cmp_rate = 0.1, wavelet='haar', WTpadding_mode = 'periodization',bn=True, growth_rate=48, bottle_neck=[4,4,4,4], block_num = 1):
        ...
        for i in range(block_num):
            ...
            if (i+1) < block_num:
                ...
                trans_out_chs = int((out_chs+trans_chs)*0.5)
                self.trans_layer_list.add_module('translayer{}'.format(i),nn.Sequential(
                    # nn.Conv2d(trans_in_chs,trans_out_chs, kernel_size=3,padding=1,stride=2,bias=False),
                    # nn.BatchNorm2d(trans_out_chs),nn.LeakyReLU()
                    nn.Conv2d(trans_in_chs,trans_out_chs, kernel_size=1,bias=False),
                    nn.BatchNorm2d(trans_out_chs),nn.LeakyReLU(),
                    nn.AvgPool2d(2,2)
                    ))
...
if __name__ == '__main__':
    ...
    model = MLWBDN(15,9,cmp_rate=0.1,wavelet='haar', growth_rate=36,bottle_neck=[4,4,4,4],bn=True,block_num = 3)
    model.train()
