...
class S3KAIResNet(nn.Module):
    ...
    def forward(self, X):  ##X:(N,B,H,W)
        X = X.permute(0,2,3,1).unsqueeze(dim = 1).contiguous() ## (N,B,H,W)-->(N,C=1,H,W,B)
...
if __name__ == '__main__':
    ...
    NUM_CLASS = 15
    PATCHSIZE = 11
    model = S3KAIResNet(Band, NUM_CLASS, 2)
    # inpt = torch.randn((1,1,PATCHSIZE,PATCHSIZE,Band))
    # text_ = stat(model,input_size=(PATCHSIZE,PATCHSIZE,Band))
    # print(text_)
    with torch.no_grad():
        summary(model, input_data=torch.randn((1,Band,PATCHSIZE,PATCHSIZE)),col_names=['num_params','kernel_size','mult_adds','input_size','output_size'],col_width=10,row_settings=['var_names'],depth=10,device='cpu')
    # summary(model.to('cuda'),input_size=(1,PATCHSIZE,PATCHSIZE,Band))
