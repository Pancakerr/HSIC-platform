...
class WT3DForward(nn.Module):
    def __init__(self,J, wave, mode):
        super().__init__()
        self.DWT2D = DWTForward(J, wave, mode)
        self.DWT1D1 = DWT1DForward(J, wave, mode)
        self.DWT1D2 = DWT1DForward(J, wave, mode)
        self.DWT1D3 = DWT1DForward(J, wave, mode)
        self.DWT1D4 = DWT1DForward(J, wave, mode)
        
    
    def forward(self,x):
        x_dwt = []
        x_LL,[x_H] = self.DWT2D(x) ##x_H:(Batch x Channels x 3 x Width x Height)
        [N, C, H, W] =x_LL.shape
        x_LLL,[x_LLH] =self.DWT1D1(x_LL.view(N,C,-1).permute(0,2,1))
        x_LHL,[x_LHH] =self.DWT1D2(x_H[:,:,0,...].view(N,C,-1).permute(0,2,1))
        x_HLL,[x_HLH] =self.DWT1D3(x_H[:,:,1,...].view(N,C,-1).permute(0,2,1))
        x_HHL,[x_HHH] =self.DWT1D4(x_H[:,:,2,...].view(N,C,-1).permute(0,2,1))
        x_dwt.append(torch.cat([x_LLL.permute(0,2,1).view(N,-1,H,W),x_LLH.permute(0,2,1).view(N,-1,H,W)],dim = 1))
        x_dwt.append(torch.cat([x_LHL.permute(0,2,1).view(N,-1,H,W),x_LHH.permute(0,2,1).view(N,-1,H,W)],dim = 1))
        x_dwt.append(torch.cat([x_HLL.permute(0,2,1).view(N,-1,H,W),x_HLH.permute(0,2,1).view(N,-1,H,W)],dim = 1))
        x_dwt.append(torch.cat([x_HHL.permute(0,2,1).view(N,-1,H,W),x_HHH.permute(0,2,1).view(N,-1,H,W)],dim = 1))
        return x_dwt
...
class MFFBlock(nn.Module):
    def __init__(self, in_chs, cmp_rate, wavelet='haar', WTpadding_mode = 'periodization',bn=False, growth_rate=64, bottle_neck=[0,0,4,4], block_num = 1):
        ...
        self.in_chs = in_chs
        self.DWT=WT3DForward(J=1, wave=wavelet, mode=WTpadding_mode)
        tr_in_chs=self.DWT(torch.zeros(1,in_chs,3,3))[0].shape[1]
        out_chs = int(tr_in_chs*cmp_rate)
        self.Transitionlayer = nn.Sequential()
        for i in range(4):
            self.Transitionlayer.add_module("transitionglayer{}".format(i),nn.Sequential(
                                                nn.Conv2d(tr_in_chs, out_chs, kernel_size=1, padding=0, stride=1),
        ...
        self.SADFBlock = SADFBlock(out_chs, growth_rate, bottle_neck,layercount=4)
    ...
    def get_final_channels(self):
        x = torch.zeros(1,self.in_chs,10,10)
        with torch.no_grad():
            x =self.forward(x)
        return x.shape[1]
    ...
    def forward(self, x):
        x_dwt = self.DWT(x)
        for i in range(4):
            x_dwt[i] = self.Transitionlayer[i](x_dwt[i])
    ...
    def __init__(self, in_chs, GAPout_num, num_class, cmp_rate = 0.1, wavelet='haar', WTpadding_mode = 'periodization',bn=True, growth_rate=48, bottle_neck=[4,4,4,4], block_num = 1):
        ...
        for i in range(block_num):
            ...
            self.classifier_list.add_module('classifier_{}'.format(i+1),
                                            classifier(out_chs, GAPout_num, bn = bn, block_num = i+1))
            # self.classifier_list.add_module('classifier_{}'.format(i+1),
...
if __name__ == '__main__':
    ...
    model = MLWBDN(103,36,9,cmp_rate=0.2,wavelet='haar', growth_rate=36,bn=True,block_num = 3)
    model.train()
    x=torch.randn(10,103,25,25)
    target=torch.randint(0,9,(10,))
    y=model(x)
    criterion = nn.CrossEntropyLoss()
    out=criterion(y,target)
    summary(model, input_data=x,col_names=['num_params','kernel_size','mult_adds','input_size','output_size'],col_width=10,row_settings=['var_names'],depth=4)
